...
const provider = (
  ...
  (process.env.GEMINI_API_KEY ? 'gemini' : process.env.GROQ_API_KEY ? 'groq' : process.env.XAI_API_KEY ? 'xai' : 'openrouter')
).toLowerCase()
...
const fallbackModels = isXai || isGroq || isGemini
...
app.get('/api/health', async (_req, res) => {
  ...
  let networkError = ''
  const providerName = isXai ? 'xAI' : isGroq ? 'GroqCloud' : isGemini ? 'Gemini' : 'OpenRouter'
  ...
  res.json({
    ok: true,
    provider: isXai ? 'xai' : isGroq ? 'groq' : isGemini ? 'gemini' : 'openrouter',
    model,
...
app.post('/api/chat', async (req, res) => {
  if (!apiKey) {
    res.json({
      fallback: true,
      content:
        `Local simulation active. Add ${isXai ? 'XAI_API_KEY' : isGroq ? 'GROQ_API_KEY' : isGemini ? 'GEMINI_API_KEY' : 'OPENROUTER_API_KEY'} to .env and run npm run server for live AI responses.`,
    })
    return
  }
  ...
  try {
    ...
    const modelCandidates = hasImage && !isXai && !isGroq && !isGemini ? Array.from(new Set([...visionModels, ...fallbackModels])) : fallbackModels
    ...
    res.status(502).json({ error: `No ${isXai ? 'Grok' : isGroq ? 'Groq' : isGemini ? 'Gemini' : 'OpenRouter'} model returned content. ${errors.join(' | ')}` })
  } catch (error) {
    const causeCode = error?.cause?.code
    const message =
      causeCode === 'EACCES'
        ? `Network access is blocked for this Node process. Run npm run server from your normal Windows terminal, or allow outbound access to ${isXai ? 'api.x.ai' : isGroq ? 'api.groq.com' : isGemini ? 'generativelanguage.googleapis.com' : 'openrouter.ai'}.`
        : error instanceof Error
          ? error.message
